 ...
 * @hidden
 */
class Parser<E> extends ContextAwareBase {
  ...
  Node T() throws ScanException {
    ...
    case Token.LITERAL:
      advanceTokenPointer();
      return new Node(Node.LITERAL, t.getValue(), t.getPosition());
    case Token.PERCENT:
      advanceTokenPointer();
  ...
  FormattingNode C() throws ScanException {
    ...
    case Token.COMPOSITE_KEYWORD:
      advanceTokenPointer();
      return COMPOSITE(t.getValue().toString(), t.getPosition());
    default:
  ...
  FormattingNode SINGLE() throws ScanException {
    ...
    SimpleKeywordNode keywordNode = new SimpleKeywordNode(t.getValue(), t.getPosition());
  ...
  FormattingNode COMPOSITE(String keyword, int position) throws ScanException {
    CompositeNode compositeNode = new CompositeNode(keyword, position);

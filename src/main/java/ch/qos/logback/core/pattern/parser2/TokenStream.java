...
class TokenStream {
  ...
  TokenizerState lastState = state;
  int pointer = 0;
  int startOfToken = 0;
  ...
  List<Token> tokenize() throws ScanException {
    ...
    while (pointer < patternLength) {
      if (state != lastState) {
        ...
        // Record this char position as the starting position of the token.
        // If we're transitioning from the OPTION state to the LITERAL state,
        // record the position as-is since the OptionTokenizer properly
        // advances the pointer to the correct position. Otherwise, we backtrack 
        // by 1 because at this point, "pointer" has already been incremented.
        if ((lastState == TokenizerState.OPTION_STATE) && (state == TokenizerState.LITERAL_STATE)) {
          startOfToken = pointer;
        } else {
          startOfToken = Math.max(0, pointer - 1);
        }
    ...
    switch (state) {
      case LITERAL_STATE:
        ...
        break;
      case KEYWORD_STATE:
        tokenList.add(new Token(Token.SIMPLE_KEYWORD, buf.toString(), startOfToken));
        break;
      case RIGHT_PARENTHESIS_STATE:
  ...
  private void addValuedToken(int type, StringBuffer buf, List<Token> tokenList) {
    if (buf.length() > 0) {
      tokenList.add(new Token(type, buf.toString(), startOfToken));
      buf.setLength(0);
    }
  }
}